LDBC SNB (Social Network Benchmark) Interactive 是国际关联数据基准委员会（LDBC）制定的、用于评估图数据库在事务型在线查询（OLTP）场景性能的行业标准基准。它模拟社交网络业务，覆盖多跳、短路径等复杂查询以及数据增删改查。NeuG 在 LDBC SNB Interactive 基准测试中有世界领先的性能表现。本文给出从零开始跑通 NeuG + LDBC SNB Interactive 的复现流程。
### 环境准备
建议使用两台机器：
...
- `ldbc.snb.datagen.serializer.numUpdatePartitions`：Update Streams 分区数。通常与基准测试要求保持一致，常用值为 `48`。
...
```bash
${NEUG_WORKSPACE}/build/tools/utils/bulk_loader -g ${NEUG_WORKSPACE}/tests/resources/ldbc/configs/graph.yaml \
        -l ${NEUG_WORKSPACE}/tests/resources/ldbc/configs/import.yaml -d $DATA_DIR [-p n]
```
参数说明如下：
...
需要特别注意，`-d`指定的是**输出目录**，不是原始CSV目录。原始LDBC数据集目录需要在`import.yaml`中的`loading_config.data_source.location`里配置，例如：
...
validation测试主要验证查询结果正确性。

#### benchmark测试

首先需要编辑 ${DRIVER_WORKSPACE}/driver/neug/driver/benchmark-{Scale-Factor}.properties，完成下面的一些配置：
```yaml
url=http://IP:PORT
...
`queryDir`、`updates_dir`、`parameters_dir` 需要指定绝对路径，避免因运行目录变化导致找不到文件。

● 时间压缩比 (TCR)
此参数通过压缩查询开始时间之间的间隔，来提高或降低查询频率，从而使系统能够在给定工作负载中达到其最大吞吐。该数值越小，代表的压缩比越高（例如：2.0 表示查询发送频率减慢为原来的 1/2，而 0.1 则表示加快为原来的 10 倍）。当前实验使用的TCR配置如下，
...
其中operation count 和 warm随TCR进行调节，benchmark测试分为预热（warmup）和性能测试（benchmark）两个阶段，相应的查询数量要满足预热阶段至少持续30分钟，性能测试阶段跑满2小时，但不超过2小时15分钟。
...
● 查询准时率
由上面的介绍可以知道，TCR调越小，吞吐可能越大，那么是否TCR越小越好？这就涉及一个查询准时率的概念，可以理解为每一个查询要尽量在1s内返回，否则视为超时。基准测试要求在一轮测试中，超时查询比例不能超过5%，否则认为性能结果无效，此时需要上调TCR，降低查询频率，同时相应下调operation count和warmup数值。
### ACID测试
暂缺
### Recovery与持久化测试
