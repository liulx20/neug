...
class ShortestPathOperator : public IVecOperator {
 public:
	ShortestPathOperator(int start_tag, int v_alias, int path_alias,
	                     execution::LabelTriplet label, int hop_lower,
	                     int hop_upper, std::string dest_param_name,
	                     int64_t dest_const, bool use_param);

	std::string GetName() const override { return "ShortestPathOperator"; }
